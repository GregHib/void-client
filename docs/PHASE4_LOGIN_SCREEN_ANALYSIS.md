...
| 12 | `Class369_Sub3_Sub1` | JVM | commonMain | **IN PROGRESS.** AWT `ActionEvent`/`EventQueue` split out to jvm `Class369_Sub3_Sub1Statics` (`method3578`); class now import-clean. Move still **blocked**: base chain `Class369_Sub3`→`Class369`→`Interface12` + ctor-param types `Class221`/`Class341` must be common first. Foundation landed: `Interface12` and its return type `Class223` **moved to commonMain** (2026-06-12), `Class223` jvm coupling (`Npc`/`Player`/`Client`, the `Class221` handle) split to `Class223Statics`. Remaining hard blocker: `Class369`'s ctor + stored fields hard-reference `Class221?`/`Class341?` (both purely jvm) — the Class369 chain cannot move until those two move (a larger instance-method split, deferred). |

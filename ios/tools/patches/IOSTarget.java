...
import com.mobidevelop.robovm.org.apache.commons.io.FilenameUtils;
import com.mobidevelop.robovm.org.apache.commons.io.filefilter.AndFileFilter;
import com.mobidevelop.robovm.org.apache.commons.io.filefilter.PrefixFileFilter;
import com.mobidevelop.robovm.org.apache.commons.io.filefilter.RegexFileFilter;
import com.mobidevelop.robovm.org.apache.commons.io.filefilter.SuffixFileFilter;
...
public class IOSTarget extends AbstractTarget {
    ...
    protected void doBuild(File outFile, List<String> ccArgs,
        ...
        // Link as iOS (RoboVM vm libs are iOS-platform). tvOS stamp happens
        // post-link via vtool before codesign — see stampTvOSBinary().
    ...
    protected void prepareInstall(File installDir) throws IOException {
        ...
        if (isDeviceArch(arch)) {
            ...
            // RoboVM rt is iOS-linked; re-stamp Mach-O platform to tvOS so Apple TV accepts the binary.
            if (SDK.isTvOSBuild()) {
                stampTvOSBinary(new File(installDir, getExecutable()));
            }
    ...
    protected void prepareLaunch(File appDir) throws IOException {
        ...
        // Simulator launch path — stamp before ad-hoc codesign (device uses prepareInstall).
        if (SDK.isTvOSBuild() && isSimulatorArch(arch)) {
            stampTvOSBinary(new File(appDir, getExecutable()));
        }
    ...
    private void codesign(SigningIdentity identity, File entitlementsPList, boolean preserveMetadata,
        ...
        if (allocate) {
            String platform = SDK.isTvOSBuild() ? "appletvos" : "iphoneos";
            executor.addEnv("CODESIGN_ALLOCATE", ToolchainUtil.findXcodeCommand("codesign_allocate", platform));
        }
        executor.args(args);
        executor.exec();
    }
    ...
    public List<Arch> getDefaultArchs() {
        return Arrays.asList(new Arch(CpuArch.arm64));
    }
            ...
            if (swiftLibs != null && swiftLibs.length > 0){
                ...
                if (config.getOs() == OS.ios) {
                    if (config.getArch().isArm()) {
                        swiftSupportDir = new File(swiftSupportDir,
                                SDK.isTvOSBuild() ? "appletvos" : "iphoneos");
                    }
                } else {
    ...
    /**
     * Rewrite LC_BUILD_VERSION to tvOS / tvOS Simulator. Required because
     * RoboVM's static libs only ship as iOS / iOS Simulator.
     * <p>
     * Device: platform name {@code tvos}. Simulator: numeric platform {@code 8}
     * ({@code TVOSSIMULATOR}) — current Xcode {@code vtool} has no {@code tvossim}
     * string alias (unlike {@code iossim}).
     */
    private void stampTvOSBinary(File binary) throws IOException {
        String min = getMinimumOSVersion();
        String sdkVer = sdk != null ? sdk.getVersion() : min;
        // Device = "tvos"; simulator = PLATFORM_TVOSSIMULATOR (8).
        String platform = isSimulatorArch(arch) ? "8" : "tvos";
        File tmp = new File(binary.getParentFile(), binary.getName() + ".tvos");
        config.getLogger().info("Stamping %s as platform=%s minos=%s sdk=%s via vtool",
                binary, platform, min, sdkVer);
        new Executor(config.getLogger(), "xcrun")
                .args("vtool",
                        "-set-build-version", platform, min, sdkVer,
                        "-replace",
                        "-output", tmp.getAbsolutePath(),
                        binary.getAbsolutePath())
                .exec();
        if (!tmp.exists()) {
            throw new IOException("vtool failed to write " + tmp);
        }
        if (!binary.delete() || !tmp.renameTo(binary)) {
            // Fallback copy
            Files.copy(tmp.toPath(), binary.toPath(), StandardCopyOption.REPLACE_EXISTING);
            tmp.delete();
        }
    }
    ...
    protected void customizeInfoPList(NSDictionary dict) {
        if (isSimulatorArch(arch)) {
            String simPlat = SDK.isTvOSBuild() ? "AppleTVSimulator" : "iPhoneSimulator";
            dict.put("CFBundleSupportedPlatforms", new NSArray(new NSString(simPlat)));
        } else {
            String devPlat = SDK.isTvOSBuild() ? "AppleTVOS" : "iPhoneOS";
            dict.put("CFBundleSupportedPlatforms", new NSArray(new NSString(devPlat)));
            ...
            if (SDK.isTvOSBuild()) {
                // Linked against iPhoneOS.sdk but shipped as Apple TV — fix metadata.
                dict.put("DTPlatformName", "appletvos");
                dict.put("DTSDKName", "appletvos" + sdk.getVersion());
            }
    ...
    protected void createInfoPList(File dir) throws IOException {
        ...
        if (config.getIosInfoPList() != null && config.getIosInfoPList().getDictionary() != null) {
            ...
            // tvOS apps must not claim LSRequiresIPhoneOS.
            if (!SDK.isTvOSBuild()) {

...
public class Toolkit {
    ...
    public Image createImage(byte[] data) {
        ...
        UIImage ui = new UIImage(new NSData(data));
        CGImage cg = ui == null ? null : ui.getCGImage();
        if (cg == null) {
            throw new IllegalArgumentException("decode failed");
        }
        int w = (int) cg.getWidth();
        int h = (int) cg.getHeight();
        BufferedImage image = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
        ArgbBridge.copy(ui, image.peekArgb(), w, h);

...
final class SceneEditorHost {
    ...
    static String spawnAtPlayer(int objectId) throws IOException {
        ...
        return spawnAt(objectId, MicrobotWidgets.localAbsX(), MicrobotWidgets.localAbsY(),
                MicrobotWidgets.localPlane());
    }

    static SceneObject addAt(int objectId, int x, int y, int plane) {
        SceneObject probe = new SceneObject(0, objectId, x, y, 0, plane);
        SceneObjectAdapter.remove(probe);
    ...
    static String spawnNpcAt(int npcId, int absX, int absY, int plane) {
        ...
        sendServerCommand("scene_npc_spawn " + npcId + " " + absX + " " + absY + " " + plane);
    ...
    private static void sendServerCommand(String command) {
        try {
            ...
            ParticleSystem packet = ParticleShader.createOutboundPacket(
                    ...
                    -106);
            packet.particleBuffer.writeByte(false, command.length() + 3);
            packet.particleBuffer.writeByte(false, 0);
            packet.particleBuffer.writeByte(false, 0);
            packet.particleBuffer.writeString((byte) -5, command);

...
*/
class ModelKeyBuilder : LinkedListNode() {
    ...
    fun method2803(widgetDefinition: WidgetDefinition?, compositeNpcModelBuilder: CompositeNpcModelBuilder?, i: Int, i_10_: Int, i_11_: Int, i_12_: Int, i_13_: Int, var_renderer: Renderer?, bool: Boolean, i_14_: Int): AbstractModel? {
        try {
            ...
            if (abstractModel == null || var_renderer!!.method3667(abstractModel.ua(), i_15_) != 0) {
                ...
                var i_30_ = 0
                while (`is`!!.size > i_30_) {
                    if (`is`[i_30_] != -1 && !Exception_Sub1.aModelHeaderCache_112!!.method1940(-91, `is`[i_30_]).method1565(bool, -1)) bool_29_ = true
                    i_30_++
                }
                if (bool_29_) return null
                val modelDefinitions = arrayOfNulls<ModelDefinition>(`is`.size)
                for (i_31_ in `is`.indices) {
                    if (`is`[i_31_] != -1) modelDefinitions[i_31_] = Exception_Sub1.aModelHeaderCache_112!!.method1940(CameraSplineNode.Companion.method2955(i_14_, -112), `is`[i_31_]).method1558(bool, false)
                }

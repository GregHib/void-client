object RasterSpriteStatics {
    @JvmField
            ...
            fun method168(i: Byte) {
                do {
                    if (!VorbisOggDecoderStatics.aPrivilegedOperationWorker_8992!!.aBoolean3794) {
                        try {
                            try {
                                val runtime = Runtime.getRuntime()
                                val var_long = runtime.maxMemory()
                                TextureDefinitionCacheStatics.anInt2964 = 1 + (var_long / 1048576L).toInt()
                    ...
                    TextureDefinitionCacheStatics.anInt2964 = 96
                } while (false)
                val i_2_ = 15 % (i / 51)
                anInt5205++
            }

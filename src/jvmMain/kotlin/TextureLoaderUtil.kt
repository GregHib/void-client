...
object TextureLoaderUtil {
    ...
    fun method2547(i: Int, i_0_: Byte): Boolean {
        ...
        if (WarpTextureNodeStatics.aWidgetComponentArrayArray9427!![i] == null) WarpTextureNodeStatics.aWidgetComponentArrayArray9427!![i] = arrayOfNulls<WidgetComponent>(i_1_)
        var i_2_ = 0
        while (i_1_ > i_2_) {
            if (WarpTextureNodeStatics.aWidgetComponentArrayArray9427!![i]!![i_2_] == null) {
                val `is` = RenderConfigFactoryStatics.aJs5Archive_2978!!.method410(i_0_.toInt() xor 0x717.inv(), i, i_2_)
                if (`is` != null) {
                    val widgetComponent = (WidgetComponent().also { WarpTextureNodeStatics.aWidgetComponentArrayArray9427!![i]!![i_2_] = it })

import kotlinx.coroutines.runBlocking

/* Class59_Sub1_Sub1 - Decompiled by JODE
* Visit http://jode.sourceforge.net/
*/
class ScrollingNoiseTexture : GrayscaleNoiseTexture(8, 5, 8, 8, 2, 0.1f, 0.55f, 3.0f) {
    private lateinit var aByteArray8657: ByteArray
    override fun method550(i: Int, i_3_: Byte, i_4_: Byte) {
        if (i_4_.toInt() == 14) {
            anInt8654++
            var i_5_ = i * 2
            aByteArray8657!![i_5_++] = (-1).toByte()
            val i_6_ = i_3_.toInt() and 0xff
            aByteArray8657!![i_5_] = (i_6_ * 3 shr 5).toByte()
        }
    }

    fun method559(i: Int, i_8_: Int, i_9_: Byte, i_10_: Int): ByteArray? {
        aByteArray8657 = ByteArray(2 * (i_8_ * i) * i_10_)
        anInt8655++
        this.method542(i_8_, i, 0, i_10_)
        return aByteArray8657
    }

    companion object {
        @JvmField
        var anInt8652: Int = 0
        @JvmField
        var anInt8653: Int = 0
        @JvmField
        var anInt8654: Int = 0
        @JvmField
        var anInt8655: Int = 0
        @JvmField
        var anInt8656: Int = 0
        @JvmField
        var anInt8658: Int = 0

        @JvmStatic
        fun method556(bool: Boolean) {
            anInt8656++
            if (ActorEntity.aResourceLoaderThread_897 != null) ActorEntity.aResourceLoaderThread_897!!.method2319((-75).toByte())
            if (bool == false) {
                if (TerrainShadowBuilderGl2.job != null) {
                    while (true) {
                        try {
                            runBlocking {
                                TerrainShadowBuilderGl2.job!!.join()
                            }
                            break
                        } catch (interruptedexception: InterruptedException) {
                            /* empty */
                        }
                    }
                }
            }
        }

        @JvmStatic
        fun method557(i: Int, i_0_: Byte): AbstractProceduralTextureNode? {
            val i_1_ = 22 / ((i_0_ - 37) / 63)
            anInt8653++
            val i_2_ = i
            while_42_@ do {
                while_41_@ do {
                    while_40_@ do {
                        while_39_@ do {
                            while_38_@ do {
                                while_37_@ do {
                                    while_36_@ do {
                                        while_35_@ do {
                                            while_34_@ do {
                                                while_33_@ do {
                                                    while_32_@ do {
                                                        while_31_@ do {
                                                            while_30_@ do {
                                                                while_29_@ do {
                                                                    while_28_@ do {
                                                                        while_27_@ do {
                                                                            while_26_@ do {
                                                                                while_25_@ do {
                                                                                    while_24_@ do {
                                                                                        while_23_@ do {
                                                                                            while_22_@ do {
                                                                                                while_21_@ do {
                                                                                                    while_20_@ do {
                                                                                                        while_19_@ do {
                                                                                                            while_18_@ do {
                                                                                                                while_17_@ do {
                                                                                                                    while_16_@ do {
                                                                                                                        while_15_@ do {
                                                                                                                            while_14_@ do {
                                                                                                                                while_13_@ do {
                                                                                                                                    while_12_@ do {
                                                                                                                                        while_11_@ do {
                                                                                                                                            while_10_@ do {
                                                                                                                                                while_9_@ do {
                                                                                                                                                    while_8_@ do {
                                                                                                                                                        while_7_@ do {
                                                                                                                                                            while_6_@ do {
                                                                                                                                                                while_5_@ do {
                                                                                                                                                                    do {
                                                                                                                                                                        if (i_2_ != 0) {
                                                                                                                                                                            if (i_2_ != 1) {
                                                                                                                                                                                if (i_2_ != 2) {
                                                                                                                                                                                    if (i_2_ != 3) {
                                                                                                                                                                                        if (i_2_ != 4) {
                                                                                                                                                                                            if (i_2_ != 5) {
                                                                                                                                                                                                if (i_2_ != 6) {
                                                                                                                                                                                                    if (i_2_ != 7) {
                                                                                                                                                                                                        if (i_2_ != 8) {
                                                                                                                                                                                                            if (i_2_ != 9) {
                                                                                                                                                                                                                if (i_2_ != 10) {
                                                                                                                                                                                                                    if (i_2_ != 11) {
                                                                                                                                                                                                                        if (i_2_ != 12) {
                                                                                                                                                                                                                            if (i_2_ != 13) {
                                                                                                                                                                                                                                if (i_2_ != 14) {
                                                                                                                                                                                                                                    if (i_2_ != 15) {
                                                                                                                                                                                                                                        if (i_2_ != 16) {
                                                                                                                                                                                                                                            if (i_2_ != 17) {
                                                                                                                                                                                                                                                if (i_2_ != 18) {
                                                                                                                                                                                                                                                    if (i_2_ != 19) {
                                                                                                                                                                                                                                                        if (i_2_ != 20) {
                                                                                                                                                                                                                                                            if (i_2_ != 21) {
                                                                                                                                                                                                                                                                if (i_2_ != 22) {
                                                                                                                                                                                                                                                                    if (i_2_ != 23) {
                                                                                                                                                                                                                                                                        if (i_2_ != 24) {
                                                                                                                                                                                                                                                                            if (i_2_ != 25) {
                                                                                                                                                                                                                                                                                if (i_2_ != 26) {
                                                                                                                                                                                                                                                                                    if (i_2_ != 27) {
                                                                                                                                                                                                                                                                                        if (i_2_ != 28) {
                                                                                                                                                                                                                                                                                            if (i_2_ != 29) {
                                                                                                                                                                                                                                                                                                if (i_2_ != 30) {
                                                                                                                                                                                                                                                                                                    if (i_2_ != 31) {
                                                                                                                                                                                                                                                                                                        if (i_2_ != 32) {
                                                                                                                                                                                                                                                                                                            if (i_2_ != 33) {
                                                                                                                                                                                                                                                                                                                if (i_2_ != 34) {
                                                                                                                                                                                                                                                                                                                    if (i_2_ != 35) {
                                                                                                                                                                                                                                                                                                                        if (i_2_ != 36) {
                                                                                                                                                                                                                                                                                                                            if (i_2_ != 37) {
                                                                                                                                                                                                                                                                                                                                if (i_2_ != 38) {
                                                                                                                                                                                                                                                                                                                                    if (i_2_ != 39) break@while_42_
                                                                                                                                                                                                                                                                                                                                } else break@while_40_
                                                                                                                                                                                                                                                                                                                                break@while_41_
                                                                                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                                                                                        } else break@while_38_
                                                                                                                                                                                                                                                                                                                        break@while_39_
                                                                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                                                                } else break@while_36_
                                                                                                                                                                                                                                                                                                                break@while_37_
                                                                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                                                                        } else break@while_34_
                                                                                                                                                                                                                                                                                                        break@while_35_
                                                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                                                } else break@while_32_
                                                                                                                                                                                                                                                                                                break@while_33_
                                                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                                                        } else break@while_30_
                                                                                                                                                                                                                                                                                        break@while_31_
                                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                                } else break@while_28_
                                                                                                                                                                                                                                                                                break@while_29_
                                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                                        } else break@while_26_
                                                                                                                                                                                                                                                                        break@while_27_
                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                } else break@while_24_
                                                                                                                                                                                                                                                                break@while_25_
                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                        } else break@while_22_
                                                                                                                                                                                                                                                        break@while_23_
                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                } else break@while_20_
                                                                                                                                                                                                                                                break@while_21_
                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                        } else break@while_18_
                                                                                                                                                                                                                                        break@while_19_
                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                } else break@while_16_
                                                                                                                                                                                                                                break@while_17_
                                                                                                                                                                                                                            }
                                                                                                                                                                                                                        } else break@while_14_
                                                                                                                                                                                                                        break@while_15_
                                                                                                                                                                                                                    }
                                                                                                                                                                                                                } else break@while_12_
                                                                                                                                                                                                                break@while_13_
                                                                                                                                                                                                            }
                                                                                                                                                                                                        } else break@while_10_
                                                                                                                                                                                                        break@while_11_
                                                                                                                                                                                                    }
                                                                                                                                                                                                } else break@while_8_
                                                                                                                                                                                                break@while_9_
                                                                                                                                                                                            }
                                                                                                                                                                                        } else break@while_6_
                                                                                                                                                                                        break@while_7_
                                                                                                                                                                                    }
                                                                                                                                                                                } else break
                                                                                                                                                                                break@while_5_
                                                                                                                                                                            }
                                                                                                                                                                        } else return BrightnessTextureNode()
                                                                                                                                                                        return ConstantColourTextureNode()
                                                                                                                                                                    } while (false)
                                                                                                                                                                    return BlankTextureNode()
                                                                                                                                                                } while (false)
                                                                                                                                                                return SourceRowTextureNode()
                                                                                                                                                            } while (false)
                                                                                                                                                            return CellNoiseTextureNode()
                                                                                                                                                        } while (false)
                                                                                                                                                        return BoxBlurTextureNode()
                                                                                                                                                    } while (false)
                                                                                                                                                    return ClampTextureNode()
                                                                                                                                                } while (false)
                                                                                                                                                return CombineTextureNode()
                                                                                                                                            } while (false)
                                                                                                                                            return CurveTextureNode()
                                                                                                                                        } while (false)
                                                                                                                                        return FlipTextureNode()
                                                                                                                                    } while (false)
                                                                                                                                    return GradientLookupEffect()
                                                                                                                                } while (false)
                                                                                                                                return ColourKeyTextureNode()
                                                                                                                            } while (false)
                                                                                                                            return SineWaveTextureNode()
                                                                                                                        } while (false)
                                                                                                                        return NoiseTextureNode()
                                                                                                                    } while (false)
                                                                                                                    return RadialGradientMaskEffect()
                                                                                                                } while (false)
                                                                                                                return VoronoiNoiseTextureNode()
                                                                                                            } while (false)
                                                                                                            return DirectionalStripeTextureNode()
                                                                                                        } while (false)
                                                                                                        return HslAdjustTextureNode()
                                                                                                    } while (false)
                                                                                                    return SpriteRgbTextureNode()
                                                                                                } while (false)
                                                                                                return DisplaceTextureNode()
                                                                                            } while (false)
                                                                                            return TileTextureNode()
                                                                                        } while (false)
                                                                                        return BlendTextureNode()
                                                                                    } while (false)
                                                                                    return InvertTextureNode()
                                                                                } while (false)
                                                                                return WarpTextureNode()
                                                                            } while (false)
                                                                            return GrayscaleEffect()
                                                                        } while (false)
                                                                        return (ColorThresholdEffect())
                                                                    } while (false)
                                                                    return (RangeThresholdTextureNode())
                                                                } while (false)
                                                                return (GradientMapTextureNode())
                                                            } while (false)
                                                            return (TurbulenceTextureNode())
                                                        } while (false)
                                                        return (RenderListTextureNode())
                                                    } while (false)
                                                    return (BrightnessContrastEffect())
                                                } while (false)
                                                return RadialTextureNode()
                                            } while (false)
                                            return DirectionalLightTextureNode()
                                        } while (false)
                                        return NormalMapTextureNode()
                                    } while (false)
                                    return PerlinNoiseTextureNode()
                                } while (false)
                                return EdgeDetectTextureNode()
                            } while (false)
                            return TextureMapImageNode()
                        } while (false)
                        return WeaveTextureNode()
                    } while (false)
                    return ScatterTextureNode()
                } while (false)
                return SpriteTextureNode()
            } while (false)
            return null
        }

        @JvmStatic
        fun method558(i: Int): ParticleProcessor? {
            val i_7_ = -98 % ((i - -34) / 33)
            anInt8658++
            try {
                return SubsampledParticleProcessor()
            } catch (throwable: Throwable) {
                return null
            }
        }

        @JvmStatic
        fun method560(class318_sub4: SceneEntityModel?, i: Int) {
            try {
                anInt8652++
                class318_sub4!!.aClass318_Sub1_6410 = null
                val i_11_ = (class318_sub4.aClass318_Sub3Array6414)!!.size
                val i_12_ = 127 / ((i - 82) / 32)
                for (i_13_ in 0..<i_11_) class318_sub4.aClass318_Sub3Array6414!![i_13_]!!.aBoolean6401 = false
                withLock(MapElementManager.aDoublyLinkedNodeListArray3974s!!) {
                    if (MapElementManager.aDoublyLinkedNodeListArray3974s!!.size > i_11_ && CalendarUtil.anIntArray4128!![i_11_] < 200) {
                        MapElementManager.aDoublyLinkedNodeListArray3974s!![i_11_]!!.method1869(-126, class318_sub4)
                        CalendarUtil.anIntArray4128!![i_11_]++
                    }
                }
            } catch (runtimeexception: RuntimeException) {
                throw TextureLoadException.method2929(runtimeexception, ("hha.E(" + (if (class318_sub4 != null) "{...}" else "null") + ',' + i + ')'))
            }
        }
    }
}

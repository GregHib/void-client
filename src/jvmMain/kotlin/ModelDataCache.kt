import GlShadowRenderPassStatics.method3553

/* Class24 - Decompiled by JODE
* Visit http://jode.sourceforge.net/
*/
object ModelDataCache {
    @JvmField
    var anInt354: Int = 0
    @JvmField
    var aLruByteCache_355: LruByteCache? = LruByteCache(260)
    @JvmField
    var anInt356: Int = 0
    @JvmField
    var aClass318_Sub1_Sub3Array357: Array<GroundDecorEntity?>? = null
    @JvmField
    var aByteArrayArray358: Array<ByteArray?>? = arrayOfNulls<ByteArray>(1000)
    @JvmField
    var anInt359: Int = 0

    @JvmStatic
    fun method296(i: Byte) {
        if (i.toInt() != -99) method298(false, 24, -128, 22, -7)
        aLruByteCache_355 = null
        aByteArrayArray358 = null
        aClass318_Sub1_Sub3Array357 = null
    }

    @JvmStatic
    fun method297(bool: Boolean) {
        if (bool != false) aLruByteCache_355 = null
        WorldMapElementStatics.aLruByteCache_4636!!.method590(0)
        anInt356++
    }

    @JvmStatic
    fun method298(bool: Boolean, i: Int, i_0_: Int, i_1_: Int, i_2_: Int) {
        anInt354++
        if (IntHashSetStatics.aClass348_Sub51_3959!!.aClass239_Sub25_7271!!.method1829(-32350) == 0) LoadingScreenImageNodeStatics.method3177(-98, false)
        else {
            CollisionMapRegionStatics.anInt1720 = IntHashSetStatics.aClass348_Sub51_3959!!.aClass239_Sub25_7271!!.method1829(-32350)
            method3553(true, 103.toByte(), 0)
        }
        SceneNodeDequeStatics.aBoolean1500 = bool
        MapRegionLoaderThreadStatics.anInt4202 = i
        IdleAnimationsOptionStateStatics.anInt5909 = i_0_
        MapRegionLoaderStatics.method754(i_1_)
        if (i_2_ > -41) method297(true)
    }
}

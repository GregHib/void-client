import MinimapFlagRenderer.method1083
import FrameStatsResetStatics.method1131
import GlFramebufferBlitterStatics.method282
import RsaPacketDecoderStatics.method2672
import SoundBankPatchStatics.method2929
import KaraokeSubtitleStreamStatics.method2988
import HslAdjustTextureNodeStatics.method3126
import InvertTextureNodeStatics.method3134
import CacheEntryReferenceStatics.method3196
import LinkedListNodeStatics.anInt4290
import MapRegionLoaderStatics.method749

object LoadingScreenImageNodeStatics {
    @JvmField
    var aBoolean9498: Boolean = false
    var anInt9500: Int = 0

    @JvmField
    var anInt9501: Int

    @JvmField
    var aShortArray9502: ShortArray?
    var aIntRange_9503: IntRange?

    @JvmField
    var anInt9504: Int = 0
    var anIntArray9505: IntArray? = IntArray(3)
    var anInt9506: Int = 0
    var anInt9507: Int = 0
    var anInt9508: Int = 0
    var anInt9509: Int = 0

    fun method3174(i: Int, i_0_: Int, i_1_: Int, bool: Boolean, i_2_: Byte, i_3_: Int) {
        var i = i
        var i_0_ = i_0_
        var i_1_ = i_1_
        var i_3_ = i_3_
        if (i_1_ < 1) i_1_ = 1
        anInt9507++
        if (i_3_ < 1) i_3_ = 1
        var i_4_ = i_3_ - 334
        if (i_4_ < 0) i_4_ = 0
        else if (i_4_ > 100) i_4_ = 100
        var i_5_ = ((-AbstractMenuEntryStatics.aShort1700 + HeapDiagnosticsHolder.aShort2250) * i_4_ / 100 + AbstractMenuEntryStatics.aShort1700)
        if (i_5_ < CacheEntryReferenceStatics.aShort9555) i_5_ = CacheEntryReferenceStatics.aShort9555.toInt()
        else if (ColourAdjustmentStatics.aShort851 < i_5_) i_5_ = ColourAdjustmentStatics.aShort851.toInt()
        var i_6_ = 512 * i_5_ * i_3_ / (i_1_ * 334)
        if (i_6_ >= FixedFunctionWaterPassStatics.aShort7355) {
            if (i_6_ > RefCountedHandleStatics.aShort2269) {
                i_6_ = RefCountedHandleStatics.aShort2269.toInt()
                i_5_ = i_1_ * i_6_ * 334 / (i_3_ * 512)
                if (i_5_ < CacheEntryReferenceStatics.aShort9555) {
                    i_5_ = CacheEntryReferenceStatics.aShort9555.toInt()
                    val i_7_ = i_6_ * (i_1_ * 334) / (i_5_ * 512)
                    val i_8_ = (-i_7_ + i_3_) / 2
                    if (bool) {
                        FacingDirectionNodeStatics.aRenderer6654!!.la()
                        FacingDirectionNodeStatics.aRenderer6654!!.method3675(i_1_, (-125).toByte(), i, i_0_, i_8_, -16777216)
                        FacingDirectionNodeStatics.aRenderer6654!!.method3675(i_1_, (-125).toByte(), i, i_3_ + i_0_ - i_8_, i_8_, -16777216)
                    }
                    i_0_ += i_8_
                    i_3_ -= 2 * i_8_
                }
            }
        } else {
            i_6_ = FixedFunctionWaterPassStatics.aShort7355.toInt()
            i_5_ = i_6_ * (i_1_ * 334) / (i_3_ * 512)
            if (ColourAdjustmentStatics.aShort851 < i_5_) {
                i_5_ = ColourAdjustmentStatics.aShort851.toInt()
                val i_9_ = i_5_ * i_3_ * 512 / (334 * i_6_)
                val i_10_ = (i_1_ + -i_9_) / 2
                if (bool) {
                    FacingDirectionNodeStatics.aRenderer6654!!.la()
                    FacingDirectionNodeStatics.aRenderer6654!!.method3675(i_10_, (-125).toByte(), i, i_0_, i_3_, -16777216)
                    FacingDirectionNodeStatics.aRenderer6654!!.method3675(i_10_, (-125).toByte(), -i_10_ + i + i_1_, i_0_, i_3_, -16777216)
                }
                i += i_10_
                i_1_ -= i_10_ * 2
            }
        }
        TerrainChunkBuilderStatics.anInt1550 = i_3_ * i_5_ / 334
        RandomAccessFileOnDiskStatics.anInt3047 = i_0_
        GroundDecorRendererStatics.anInt3643 = i_3_.toShort().toInt()
        WidgetComponentNodeStatics.anInt4656 = i_1_.toShort().toInt()
        LocalizedTextTripleStatics.anInt3764 = i
        val i_11_ = -3 % ((-8 - i_2_) / 56)
    }

    fun method3175(i: Byte) {
        if (!method3196(WorldMapRendererStatics.anInt4674, -114) && !method2672(WorldMapRendererStatics.anInt4674, -103)) {
            val i_12_ = ((LocalPlayerState.aPlayer_1907!!.anIntArray10320!![0]) shr 3)
            val i_13_ = ((LocalPlayerState.aPlayer_1907!!.anIntArray10317!![0]) shr 3)
            if (i_12_ < 0 || GlCubemapLightPassStatics.anInt7319 shr 3 <= i_12_ || i_13_ < 0 || (RangeThresholdTextureNodeStatics.anInt9109 shr 3 <= i_13_)) AwtKeyboardListenerStatics.method2703(0, GlCubemapLightPassStatics.anInt7319 shr 4, (RangeThresholdTextureNodeStatics.anInt9109 shr 4), (-13).toByte())
            else AwtKeyboardListenerStatics.method2703(5000, i_12_, i_13_, (-13).toByte())
        } else AwtKeyboardListenerStatics.method2703(5000, GlslMaterialPassStatics.anInt6246 shr 12, CompositeRgbNoiseTextureStatics.anInt8685 shr 12, (-13).toByte())
        anInt9509++
        method3134(-1)
        method2988(true)
        val i_14_ = -8 % ((-30 - i) / 38)
        method3126(98.toByte())
        method1131(117)
    }

    @JvmStatic
    fun method3176(bool: Boolean) {
        anIntArray9505 = null
        aShortArray9502 = null
        if (bool != true) aShortArray9502 = null
        aIntRange_9503 = null
    }

    @JvmStatic
    fun method3177(i: Int, bool: Boolean) {
        if (!bool || MapRegionLoaderStatics.aClass348_Sub42_Sub14_1243 == null) GroundDecorOptionStateStatics.anInt5976 = -1
        else GroundDecorOptionStateStatics.anInt5976 = (MapRegionLoaderStatics.aClass348_Sub42_Sub14_1243!!.anInt9628)
        anInt9500++
        MapRegionLoaderStatics.aClass348_Sub42_Sub14_1243 = null
        HeapDiagnosticsHolder.aWidgetComponent_2249 = null
        anInt4290 = 0
        RegionSceneLoader.aNodeDeque_3705 = null
        method749()
        MapRegionLoaderStatics.aNodeDeque_1254!!.method1996(127)
        GlBufferObjectStatics.aSystemFontGlyphs_4754 = null
        HeapDiagnosticsHolder.aSystemFontGlyphs_2252 = null
        TooltipComponentRendererStatics.aAbstractModelRenderer_4643 = null
        MinimapSpriteRendererStatics.aSystemFontGlyphs_4921 = null
        PingHostListHolder.aSystemFontGlyphs_2333 = null
        ModelVertexColorBufferStatics.aSystemFontGlyphs_6581 = null
        GlShaderObjectHandleStatics.aSystemFontGlyphs_521 = null
        MapRegionLoaderStatics.aSmoothingBuffer_1246 = null
        GlRectangleTextureStatics.aSystemFontGlyphs_8644 = null
        ColourAdjustmentStatics.anInt859 = -1
        if (i <= -44) {
            TextureMetadataProviderStatics.anInt4609 = -1
            WorldMapRendererStatics.aSystemFontGlyphs_4672 = null
            if (MapRegionLoaderStatics.aConfigDefinitionLoader_1238 != null) {
                MapRegionLoaderStatics.aConfigDefinitionLoader_1238!!.method1219(7851)
                MapRegionLoaderStatics.aConfigDefinitionLoader_1238!!.method1220((-100).toByte(), 64, 128)
            }
            if (MapRegionLoaderStatics.aTextureDefinitionLoader_1242 != null) MapRegionLoaderStatics.aTextureDefinitionLoader_1242!!.method1178(64, 1, 64)
            if (MapRegionLoaderStatics.aSeqDefinitionCache_1245 != null) MapRegionLoaderStatics.aSeqDefinitionCache_1245!!.method2014(64, true)
            RenderConfigFactoryStatics.aVarbitDefLoader_2981!!.method1444(64, -124)
        }
    }

    fun method3178(js5Archive: Js5Archive?, i: Int, `is`: IntArray?, js5Archive_15_: Js5Archive?) {
        try {
            anInt9506++
            if (`is` != null) AbstractTileShapeStatics.anIntArray6547 = `is`
            TerrainTileStatics.aJs5Archive_4585 = js5Archive
            if (i >= -6) method3177(-36, true)
            GlShaderObjectHandleStatics.aJs5Archive_518 = js5Archive_15_
        } catch (runtimeexception: RuntimeException) {
            throw method2929(runtimeexception, ("lk.F(" + (if (js5Archive != null) "{...}" else "null") + ',' + i + ',' + (if (`is` != null) "{...}" else "null") + ',' + (if (js5Archive_15_ != null) "{...}" else "null") + ')'))
        }
    }

    fun method3179(i: Int) {
        CacheStateResetter.method175((-50).toByte())
        anInt9508++
        val i_16_ = IntHashSetStatics.aClass348_Sub51_3959!!.aClass239_Sub3_7222!!.method1727(-32350)
        do {
            if (i_16_ == 2) {
                method1083(FacingDirectionNodeStatics.aRenderer6654, 100, 9.toByte(), HardCacheEntryReferenceStatics.anInt10432, 100, LocTypeDefinitionStatics.anInt4017)
                break
            } else if (i_16_ != 3) break
            method282(2, FacingDirectionNodeStatics.aRenderer6654, LocTypeDefinitionStatics.anInt4017, 73.toByte(), Js5ArchiveStatics.anInt666, PerlinTextureProviderStatics.anInt5812, HardCacheEntryReferenceStatics.anInt10432, 2)
        } while (false)
        if (IntHashSetStatics.aClass348_Sub51_3959!!.aClass239_Sub3_7222!!.method1735(false)) TerrainTileStatics.method3980(i + 93, ParticleSystemRendererStatics.aCanvas3869!!)
        if (FacingDirectionNodeStatics.aRenderer6654 != null) BufferToggleStateStatics.method2196((-9).toByte())
        ParticleSystemRendererStatics.aBoolean3870 = i != IntHashSetStatics.aClass348_Sub51_3959!!.aClass239_Sub3_7222!!.method1727(-32350)
        GrayscaleNoiseTextureStatics.aBoolean5300 = IntHashSetStatics.aClass348_Sub51_3959!!.aClass239_Sub3_7222!!.method1735(false)
    }

    init {
        anInt9501 = 0
        aShortArray9502 = ShortArray(256)
        aIntRange_9503 = IntRange(9, -1)
    }
}

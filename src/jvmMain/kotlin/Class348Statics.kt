/**
 * JVM-bound static state and helpers split out of [Class348]'s companion so the bare [Class348]
 * linked-list node could move to commonMain.
 *
 * These are genuine globals (sprite/model scratch arrays, the per-frame `anInt4290` progress state,
 * and the model-building helper method2711 which pulls in Class64/Class124/ha/...). They stay on JVM;
 * only [Class348] itself (the intrusive list node + its two own counters anInt4285/anInt4297) is
 * portable. Behaviour is unchanged — members keep their original names and the statics keep
